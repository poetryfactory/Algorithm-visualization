...
MyGraphicsView::MyGraphicsView()
{
    ...
    myGraphicsScene->setSceneRect(0,0,1183,800);


    this->setScene(myGraphicsScene);
    this->setFixedSize(1183,800);
    this->move(-5,-45);
    this->show();
...
void MyGraphicsView::pre(MyGraphicsVexItem * head)
{
    if(head!=nullptr){
        ...
        while(!s.empty())
        {
            ...
            resultOutput->insertPlainText(" "+head->nameText);
...
void MyGraphicsView::pos(MyGraphicsVexItem * head)
{
    if(head!=nullptr){
        ...
        while(!col.empty())
        {
            ...
            resultOutput->insertPlainText(" "+col.top()->nameText);
            resultOutput->setFont(QFont("Segoe Print",22,QFont::Bold));
            col.pop();
        }
    }

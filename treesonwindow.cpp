...
void TreeSonWindow::paintEvent(QPaintEvent *)
{
...
void TreeSonWindow::createNew()
{
    gv->init();
}

void TreeSonWindow::build()
{
    gv->buildTree();
}

void TreeSonWindow::build2()
{
    gv->buildTree2();
}

void TreeSonWindow::pre()
{
    gv->resultOutput->clear();
    gv->pre(gv->root);
}

void TreeSonWindow::in()
{
    gv->resultOutput->clear();
    gv->in(gv->root);
}

void TreeSonWindow::pos()
{
    gv->resultOutput->clear();
    gv->pos(gv->root);
}

void TreeSonWindow::morris()
{
    gv->resultOutput->clear();
    gv->morris(gv->root);
}

void TreeSonWindow::BFS()
{
    gv->resultOutput->clear();
    gv->BFS(gv->root);
}

void TreeSonWindow::SolveMaximizeStepByDFS()
{
    gv->buildTree2();
